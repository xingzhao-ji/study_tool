...
describe("mac server", () => {
  ...
  it("serves companion UI assets", async () => {
    ...
    assert.match(scriptBody, /Choose a screenshot or crop before uploading/);
    assert.match(scriptBody, /frameStatus\.textContent = "Pairing required"/);
    ...
    assert.match(scriptBody, /contentTypeForCourseUpload/);
